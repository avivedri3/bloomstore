...
[פרק 7 — עגלה, הזמנה ומחזור חיים](#ch7)

- [7.1 עגלת קניות](#ch7-1)
- [7.2 יצירת הזמנה](#ch7-2)
- [7.3 מחזור חיים של הזמנה](#ch7-3)

[פרק 8 — נקודות הקצה](#ch8)
...
![הצהרת הסטודנטים](figures/student-declaration-1.png)

![הצעה לפרויקט גמר](figures/student-declaration-2.png)
...
- עיצוב אחיד של הממשק באמצעות ספריית MUI.
...
| --- | --- | --- |
| ממשק משתמש, ניתוב, ניהול state ועיצוב ב־MUI | אביב לייסטן | `apps/frontend` |
...
| צד לקוח | עיצוב החנות | צבע, גופן ומבנה מסך אחידים דרך ספריית MUI |
...
| --- | --- | --- |
| `users` | חשבון, הרשאות ואבטחה | `email`, `passwordHash`, `fullName`, `role`, `failedLoginAttempts`, `lockUntil` |
...
| עגלת קניות | פריטים לפני ההזמנה | שינוי כמות, הסרת פריט, מעבר לקופה |
...
![קטלוג ציבורי](figures/catalog-list-public.png)

איור 1 — סינון הקטלוג ב־listPublic
...
איור 2 — הכרטיס ב־ProductCard כשהמלאי אזל
...
המסכים בנויים מרכיבי MUI, וכל עמוד עטוף ב־`PageShell`. הכפתורים לא כתובים באותיות גדולות, הפינות מעוגלות, והצל על הכרטיס עדין. לא נוסף צבע חדש בכל מסך. כל הערכים יושבים במקום אחד, ב־`src/theme/tokens.ts`, ומשם נצבעים רכיבי הספרייה.
...
נתיבי `register` ו־`login` ציבוריים. נתיבי `logout` ו־`me` דורשים `AuthGuard('jwt')`.
...
איור 3 — הנעילה ב־AuthService.login
...
ה־JWT נחתם עם מזהה המשתמש, האימייל והתפקיד. תוקף ברירת המחדל הוא שמונה שעות (`JWT_EXPIRES_IN`).

<a id="ch5-4"></a>

### 5.4 שאר מנגנוני המודול

כל נתיב מוגן נבדק ב־JWT, והקונטרולר מקבל משתמש שכבר אומת. הסיסמה נשמרת רק כ־hash. שגיאות חוזרות כ־`{ success: false, error }`, והרשמה, התחברות, נעילה והתנתקות נרשמות ב־`auditlogs`.
...
אין בשרת מודול בשם `admin` שמכיל את כל פעולות הניהול. ההגנה משותפת: `AuthGuard` ו־`AdminGuard`. הפעולות עצמן יושבות על מוצרים, על הזמנות ועל אנליטיקה. בצד הלקוח הן מתכנסות למסך אחד.
...
![הרשאת מנהל](figures/admin-guard.png)

איור 4 — הבדיקה ב־AdminGuard
...
זו הפעולה הרגישה בלוח הניהול, כי היא משנה את סטטוס ההזמנה. הקונטרולר מוגן ב־`AdminGuard`, מחלץ את מזהה ההזמנה ואת גוף הבקשה, וקורא ל־`OrdersService.updateStatus`.
...
מעבר ל־`cancelled` מחזיר את המלאי בטרנזקציה. אחרי משלוח אי אפשר לבטל, והשירות חוסם החזרת מלאי אם הסטטוס כבר `shipped`, `delivered` או `cancelled`.
...
![בדיקת מלאי](figures/cart-stock-check.png)

איור 5 — בדיקת המלאי ב־upsertItem
...
הקופה (`CheckoutPage`) עוברת משלושה שלבים בממשק אחד: סיכום העגלה, בחירה או יצירה של כתובת, ושליחת `POST /api/orders/checkout` עם `addressId` ו־`idempotencyKey`. אחרי הכתובת מופיע כפתור Place order. הסכום הוא סכום שורות ה־snapshot.
...
5. העגלה במסד מתרוקנת, ואז גם העותק הקצר בזיכרון.

![נעילת מחיר](figures/checkout-snapshot.png)

איור 6 — הורדת מלאי ונעילת מחיר ב־checkout

כישלון באמצע הטרנזקציה מחזיר את המלאי ואת העגלה לאחור. לחיצה כפולה על יצירת ההזמנה נעצרת במפתח ה־idempotency, שנשמר 24 שעות. אישור ההזמנה הוא מעבר המנהל ל־`confirmed`.
...
איור 7 — מטריצת המעברים ב־shared-types
...
ביטול אפשרי מ־`pending_payment`, מ־`confirmed` ומ־`processing`. הלקוח קורא ל־`POST /api/orders/:id/cancel`. המנהל יכול גם לבטל דרך שינוי הסטטוס. המלאי כבר ירד בקופה, ולכן הביטול מחזיר לכל שורה את הכמות שנשמרה ב־snapshot, בתוך טרנזקציה. אם מוצר היה באפס מלאי לפני ההחזרה, נשלחת התראת חזרה למלאי.

![החזרת מלאי](figures/cancel-restock.png)

איור 8 — החזרת המלאי ב־cancelAndRestock
...
טבלה 10 — חשבון מנהל ושני חשבונות לקוח
...
| 3 | מוצר לא פעיל | `isActive: false` ורענון הקטלוג | המוצר אינו מוצג | `listPublic` |
| 4 | מוצר שאזל | `stock === 0` | המוצר מוצג, אי אפשר להוסיף לעגלה, אפשר להירשם להתראה | `ProductCard`, `CartsService` |
| 5 | התמדת העגלה | הוספה, סגירת דפדפן, התחברות מחדש | הפריטים נטענים מ־MongoDB | `CartProvider` |
| 6 | חריגה מהמלאי | כמות גדולה מהמלאי | 400 `OUT_OF_STOCK` | `CartsService.upsertItem` |
| 7 | נעילת מחיר | שינוי מחיר בקטלוג אחרי קופה | סכום ההזמנה נשאר המחיר שב־snapshot | `OrdersService.checkout` |
| 8 | הזמנה כפולה | אותו `idempotencyKey` פעמיים | הזמנה אחת | `idempotencykeys` |
| 9 | גישת לקוח לניהול | `GET /api/products/admin` עם Token של לקוח | 403 | `AdminGuard` |
| 10 | מעבר סטטוס פסול | מ־`shipped` אל `cancelled` | `ILLEGAL_TRANSITION` | `canTransition`, בדיקת יחידה |
| 11 | ביטול לפני משלוח | ביטול מ־`confirmed` | המלאי חוזר | `cancelAndRestock` |
| 12 | מפתח הזרקה | שדה שמתחיל ב־`$` בגוף הבקשה | המפתח מוסר לפני השירות | `MongoSanitizeMiddleware` |

טבלה 11 — מערך הבדיקות
...
מהעבודה נלמד שהכלל העסקי צריך לשבת במקום אחד. צילום המחיר בשורת ההזמנה מונע פער בין מה שהלקוח אישר לבין מחיר שהשתנה אחר כך בקטלוג. מכונת המצבים חוסמת מעבר אסור, למשל ביטול אחרי משלוח. ביטול בלי טרנזקציה היה משנה סטטוס בלי להחזיר מלאי. הסדר Controller, אחר כך Service, ואחר כך Model הוא חלק מהנכונות: הקונטרולר לא נוגע במסד, והשירות לא בונה את תשובת ה־HTTP.
...
1. ה־Token תקף שמונה שעות, בלי מנגנון חידוש. קיצור התוקף, יחד עם Refresh בצד הלקוח, ישאיר את הלקוח מחובר לאורך הקנייה בלי להשאיר את החיבור פתוח זמן רב.
2. הבדיקות ב־CI הן בדיקות יחידה. חבילת E2E שרצה על תרחישי טבלה 11 בכל דחיפה תעבור את המסלול מהקטלוג עד ההזמנה, ולא רק פונקציה אחת.
3. הקטלוג מסונן היום לפי קטגוריה. חיפוש טקסט מלא יאפשר למצוא זר לפי שם או לפי תיאור.
4. דירוג קצר וביקורת יוכלו להופיע ליד כרטיס המוצר. המחיר שננעל בשורת ההזמנה יישאר כפי שנשמר ברגע ההזמנה.
5. `MailService` שולח היום התראה כשמוצר חוזר למלאי. אותו מנגנון יכול לשלוח דוא״ל גם כשסטטוס ההזמנה משתנה, למשל כשהיא יוצאת למשלוח.

<a id="ch10-4"></a>

### 10.4 תודות

תודה לאסף עמיר על הליווי לאורך הפרויקט. למכללת אורט סינגאלובסקי תודה על הכלים ועל הידע שנרכשו במהלך הלימודים, ולקהילת הקוד הפתוח על הספריות ועל התיעוד שעליהם עומדת BloomStore.

### מקורות

- עיצוב הממשק בספריית MUI, לפי `apps/frontend/src/theme/tokens.ts`.
