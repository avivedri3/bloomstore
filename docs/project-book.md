...
אני מצהירה כי העבודה המוגשת להלן נעשתה על ידי באופן עצמאי, על פי ידיעתי האישית, תוך שימוש בקוד, במאמרים ובמקורות אחרים על פי כללי האתיקה האקדמית ובציון מקורות מתאימים. כל הנתונים, שמות המשתמשים והסכומים המופיעים במסמך זה משמשים להדגמה לצורכי לימוד בלבד.
...
הפרויקט BloomStore הוא חנות פרחים מקוונת מלאה (Full-Stack E-Commerce), והוא נבנה כפרויקט גמר במסלול טכנאי תוכנה. המערכת מכסה את מסלול הרכישה כולו: עיון בקטלוג ציבורי, הרשמה והתחברות, עגלת קניות, בחירת כתובת משלוח, יצירת הזמנה בתשלום מדומה, ומעקב אחרי סטטוס המשלוח. מנהל החנות מנהל מלאי, מקדם הזמנות וצופה בלוח סטטיסטיקות.
...
המערכת BloomStore בנויה על ארכיטקטורת שלוש שכבות: ממשק משתמש, לוגיקה עסקית ומסד נתונים. אותו עיקרון חוזר בתוך השרת. רכיב ה־Controller מחלץ פרמטרים, מאמת קלט ב־Zod ומחזיר מעטפת `ApiResponse`. הוא אינו ניגש למסד הנתונים. רכיב ה־Service מחזיק את כללי המלאי, ההזמנה והאבטחה. רכיב ה־Model של Mongoose הוא שכבת ההתמדה.
...
| אזור | מה מוצג | כלל |
| --- | --- | --- |
| מסגרת | `Header`, תוכן, `Footer` | אין תפריט המבורגר. ניהול רק בתפריט החשבון, ורק ל־`admin` |
...
טבלה 6 — טוקני הצבע של BloomStore
...
טבלה 7 — חלוקת האחריות במודול ההזדהות
...
טבלה 8 — חלוקת האחריות במודול הניהול

<a id="ch6-2"></a>

### 6.2 מבט-על על הקונטרולרים והשירותים

בשני המודולים אותו סדר. הקונטרולר בודק את הקלט, קורא פעם אחת לשירות, ומחזיר `ok()`. השירות מחזיק את הכללים.

הזדהות, לפי סדר הפעולות ב־`AuthController`:

1. `register` — יצירת לקוח והנפקת Token.
2. `login` — בדיקת סיסמה, נעילה אחרי חמישה כישלונות, והנפקת Token.
3. `logout` — הגדלת `tokenVersion`.
4. `me` — החזרת המשתמש המחובר.

`AuthService` חותם את ה־Token בפונקציה הפרטית `issue`. ספירת הכישלונות והנעילה יושבות בשירות.

ניהול, לפי סדר העבודה של המנהל:

1. מוצרים — רשימה, יצירה, עדכון, מחיקה רכה. הקלט נבדק ב־`productInputSchema`.
2. הזמנות — רשימה, שינוי סטטוס, ביטול. שינוי סטטוס נבדק ב־`orderStatusUpdateSchema` ואז ב־`canTransition`.
3. סטטיסטיקות — `AnalyticsService.dashboard` מריץ ב־`Promise.all` הכנסות, הזמנות פתוחות, הזמנות של היום, מלאי נמוך (עד 5 יחידות ופעילים), משתמשים חדשים מששת הימים האחרונים, ומכירות לפי יום. הזמן נקבע לפי האיטית שבהן.
...
סטטוס ההזמנה הוא enum בן שישה ערכים, והמעברים הם אלו שבטבלה 9. מספר ההזמנה מגיע ממונה אטומי בקולקציית `sequences`.
...
האתגר המרכזי בשרת היה Access Token יחיד, בלי Refresh. שמונה שעות הן פשרה. תוקף קצר מנתק את הלקוח באמצע קנייה, ותוקף ארוך משאיר Token שדלף בתוקף עד שהוא פג או עד התנתקות. `tokenVersion` סוגר את הפער: התנתקות מגדילה את הגרסה ופוסלת את כל האסימונים, בלי רשימה שחורה.
